// backend/worker.js —— Cloudflare Worker + D1 数据库
// 部署到 Cloudflare Worker，绑定 D1 数据库名为 DB
...
export default {
  async fetch(request, env) {
    ...
    if (p.endsWith("/api/login") && request.method === "POST") {
      ...
      const row = await db.prepare("SELECT password FROM users WHERE id = ?").bind(d.id).first();
      if (row && row.password === d.password) return Response.json({ success: true });
    ...
    if (p.endsWith("/api/register") && request.method === "POST") {
      const d = await request.json();
      if (!/^\d{4}$/.test(d.id)) return Response.json({ success: false, message: "ID必须是四位数字" });
      const exist = await db.prepare("SELECT id FROM users WHERE id = ?").bind(d.id).first();
      if (exist) return Response.json({ success: false, message: "该ID已被注册" });
      await db.prepare("INSERT INTO users (id, email, password, is_admin) VALUES (?, ?, ?, 0)").bind(d.id, d.email, d.password).run();
    ...
    if (p.endsWith("/api/signup") && request.method === "POST") {
      const d = await request.json();
      await db.prepare("INSERT INTO signups (event_id, pilot_id, stand, callsign, aircraft) VALUES (?, ?, ?, ?, ?)").bind(d.eventId, d.pilotId, d.stand, d.callsign, d.aircraft).run();
      return Response.json({ success: true });
    }
    ...
    if (p.endsWith("/api/apply") && request.method === "POST") {
      const d = await request.json();
      await db.prepare("INSERT INTO applications (pilot_id, position, reason, loyalty, status) VALUES (?, ?, ?, ?, '待审核')").bind(d.pilotId, d.position, d.reason, d.loyalty ? 1 : 0).run();
      return Response.json({ success: true });
    }
    ...
    if (p.endsWith("/api/announce") && request.method === "POST") {
      ...
      if (!row || row.is_admin !== 1) return Response.json({ success: false, message: "无权限" });
      await db.prepare("INSERT INTO announcements (content, created_by) VALUES (?, ?)").bind(d.text, d.pilotId).run();
      return Response.json({ success: true });
    }
